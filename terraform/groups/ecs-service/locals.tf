...
locals {
  ...
  lb_listener_paths          = ["/item-groups", "/item-groups/*/items/*", "/item-group/healthcheck"]
  healthcheck_path           = "/item-group/healthcheck" # healthcheck path for item-group-workflow-api
  healthcheck_matcher        = "200"
  vpc_name                   = local.stack_secrets["vpc_name"]
  s3_config_bucket           = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename   = "item-group-workflow-api.env"
